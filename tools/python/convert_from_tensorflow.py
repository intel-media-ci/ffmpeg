...
class TFConverter:
    def __init__(self, graph_def, nodes, outfile, dump4tb):
        ...
        self.pool_activations = {'Relu':0, 'Tanh':1, 'Sigmoid':2, 'None':3, 'LeakyRelu':4}
    ...
    def dump_avg_pool_to_file(self, node, f):
        ...
        padding = node.attr['padding'].s.decode("utf-8")
        np.array([self.op2code[node.op], strides, self.pool_paddings[padding], self.pool_activations['None'],
                  in_channels, out_channels, filter_height, filter_width],dtype=np.uint32).tofile(f) # no dilations, not bias
