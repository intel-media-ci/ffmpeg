...
static void yuv444pToXyuv(const uint8_t *src[], int srcStride[],
...
static int yuv444pToXyuvWrapper(SwsContext *c, const uint8_t *src[],
                                int srcStride[], int srcSliceY, int srcSliceH,
                                uint8_t *dstParam[], int dstStride[])
{
    uint8_t *dst = dstParam[0] + dstStride[0] * srcSliceY;
    ...
    yuv444pToXyuv(source, stride, dst + srcSliceY * dstStride[0], dstStride[0],
                  srcSliceH, c->srcW);
...
#define isByteRGB(f) (             \
    ...
    if (srcFormat == AV_PIX_FMT_YUV444P) {
        if (dstFormat == AV_PIX_FMT_0YUV)
            c->convert_unscaled = yuv444pToXyuvWrapper;
    }
