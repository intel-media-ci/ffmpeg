                                 ...
                                 av_unused uint32_t size)
{
    ...
    for (i = 0; i < frame_header->tile_cols; i++) {
        pic_param.width_in_sbs_minus_1[i] =
            s->tile_group_info[i].tile_width_sbs - 1;
    }
    for (i = 0; i < frame_header->tile_rows; i++) {
        pic_param.height_in_sbs_minus_1[i] =
            s->tile_group_info[i * (frame_header->tile_cols + 1)].tile_height_sbs - 1;
    }
                                  ...
                                  uint32_t size)
{
    ...
    for (i = s->tg_start; i <= s->tg_end; i++) {
        memset(&slice_param, 0, sizeof(VASliceParameterBufferAV1));
        offset = s->tile_group_info[i].tile_group_offset +
