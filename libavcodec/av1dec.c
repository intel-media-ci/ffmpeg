...
{
    uint16_t cur_tile_num =
        s->raw_frame_header->tile_cols * s->raw_frame_header->tile_rows;
    if (s->tile_num < cur_tile_num) {
        s->tile_group_info = av_realloc_array(s->tile_group_info, cur_tile_num,
                                     ...
                                     AV1RawSequenceHeader *seq)
{
    ...
    if (seq->timing_info.num_units_in_display_tick &&
        ...
        ret = av_reduce(&avctx->framerate.den, &avctx->framerate.num,
                                            ...
                                            AV1RawFrameHeader *header)
{
    ...
    int width, height, r_width, r_height;
    int ret;

    width = header->frame_width_minus_1 + 1;
    height = header->frame_height_minus_1 + 1;
    r_width = header->render_width_minus_1 + 1;
    r_height = header->render_height_minus_1 + 1;
    ...
    ret = av_reduce(&aspect_ratio.num, &aspect_ratio.den,
                    height * r_width, width * r_height, INT_MAX);
    if (ret < 0)
        return ret;
...
static av_cold int av1_decode_init(AVCodecContext *avctx)
{
    ...
    if (avctx->extradata && avctx->extradata_size) {
        ...
        if (ret < 0) {
            av_log(avctx, AV_LOG_WARNING, "Failed to set context.\n");
            goto end;
        }
...
static int av1_frame_alloc(AVCodecContext *avctx, AV1Frame *f)
{
    ...
    frame = f->tf.f;
    frame->key_frame =
        header->frame_type == AV1_FRAME_KEY && header->show_frame == 1;
        ...
        case AV1_OBU_FRAME_HEADER:
            ...
            if (ret < 0) {
                av_log(avctx, AV_LOG_DEBUG,
                       "Update context with frame header fail:%d.\n", ret);
                goto end;
            }
