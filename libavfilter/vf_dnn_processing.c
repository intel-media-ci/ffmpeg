...
#define LOG_FORMAT_CHANNEL_MISMATCH()                       \
    av_log(ctx, AV_LOG_ERROR,                               \
           ...
           model_input->dims[DNN_CHANNEL_IDX(model_input->layout)]);
...
static int check_modelinput_inlink(const DNNData *model_input, const AVFilterLink *inlink)
{
    ...
    if (model_input->dims[DNN_HEIGHT_IDX(model_input->layout)] != -1 &&
        model_input->dims[DNN_HEIGHT_IDX(model_input->layout)] != inlink->h) {
        av_log(ctx, AV_LOG_ERROR, "the model requires frame height %d but got %d\n",
                                   model_input->dims[DNN_HEIGHT_IDX(model_input->layout)],
                                   inlink->h);
        return AVERROR(EIO);
    }
    if (model_input->dims[DNN_WIDTH_IDX(model_input->layout)] != -1 &&
        model_input->dims[DNN_WIDTH_IDX(model_input->layout)] != inlink->w) {
        av_log(ctx, AV_LOG_ERROR, "the model requires frame width %d but got %d\n",
                                   model_input->dims[DNN_WIDTH_IDX(model_input->layout)],
                                   inlink->w);
        return AVERROR(EIO);
    }
    ...
    case AV_PIX_FMT_BGR24:
        if (model_input->dims[DNN_CHANNEL_IDX(model_input->layout)] != 3) {
    ...
    case AV_PIX_FMT_NV12:
        if (model_input->dims[DNN_CHANNEL_IDX(model_input->layout)] != 1) {
            LOG_FORMAT_CHANNEL_MISMATCH();
            return AVERROR(EIO);
        }
        return 0;
