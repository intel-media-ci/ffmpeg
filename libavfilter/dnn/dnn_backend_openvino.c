...
static DNNReturnType execute_model_ov(TaskItem *task, RequestItem *request)
{
    ...
    if (task->async) {
        ...
        request->task = task;
        callback.completeCallBackFunc = infer_completion_callback;
        callback.args = request;
        status = ie_infer_set_completion_callback(request->infer_request, &callback);
        if (status != OK) {
...
DNNModel *ff_dnn_load_model_ov(const char *model_filename, const char *options, void *userdata)
{
    ...
    for (int i = 0; i < ctx->options.nireq; i++) {
        ...
        RequestItem *item = av_malloc(sizeof(*item));
        if (!item) {
            goto err;
        }
