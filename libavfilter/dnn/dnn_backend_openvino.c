...
static void infer_completion_callback(void *args)
{
    ...
    if (task->async) {
        request->task = NULL;
        if (safe_queue_push_back(task->ov_model->request_queue, request) < 0) {
...
DNNModel *ff_dnn_load_model_ov(const char *model_filename, const char *options, AVFilterContext *filter_ctx)
{
    ...
    for (int i = 0; i < ctx->options.nireq; i++) {
        ...
        if (safe_queue_push_back(ov_model->request_queue, item) < 0) {
            av_freep(&item);
            goto err;
        }
    }
                                            ...
                                            const char **output_names, uint32_t nb_output, AVFrame *out_frame)
{
    ...
    task->out_frame = out_frame;
    task->ov_model = ov_model;
    if (queue_push_back(ov_model->task_queue, task) < 0) {
        av_freep(&task);
