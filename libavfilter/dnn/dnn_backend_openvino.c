...
#define DNN_OPENVINO_SURFACE_STRING_LEN 20
#define DNN_OPENVINO_MAX_PLANE 4
...
{
    char surface_str[DNN_OPENVINO_SURFACE_STRING_LEN];
    char *port_name;
    int nb_planes;
    ...
    ov_status_e status;
    ov_tensor_t* tensor[DNN_OPENVINO_MAX_PLANE] = { NULL };
    ...
    for (int i = 0; i < nb_planes; i++) {
        ...
        if (status != OK) {
            av_log(ctx, AV_LOG_ERROR, "Failed to get input port by index.\n");
...
static int set_model_input_info(OVModel *ov_model, const char *input_name)
{
    ...
    if (ov_model->model->filter_ctx->inputs[0]->hw_frames_ctx) {
        ...
        if (hw_frame_ctx->sw_format == AV_PIX_FMT_NV12) {
            ...
            avpriv_report_missing_feature(ctx, "This sw_format is not supported.\n");
            ret = AVERROR_PATCHWELCOME;
            goto err;
        }
    ...
    if (ov_model->use_remote_context) {
        status |= ov_preprocess_preprocess_steps_convert_color(input_process_steps,
                                                                  BGR);
        status |= ov_preprocess_preprocess_steps_resize(input_process_steps, RESIZE_CUBIC);
    }
