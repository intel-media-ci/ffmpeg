...
static int config_input(AVFilterLink *inlink)
{
    ...
    ctx->scale_width = model_input.dims[DNN_WIDTH_IDX(model_input.layout)] == -1 ? inlink->w :
        model_input.dims[DNN_WIDTH_IDX(model_input.layout)];
    ctx->scale_height = model_input.dims[DNN_HEIGHT_IDX(model_input.layout)] ==  -1 ? inlink->h :
