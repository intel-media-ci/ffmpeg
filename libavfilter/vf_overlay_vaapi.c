...
static int overlay_vaapi_query_formats(AVFilterContext *ctx)
{
    ...
    ret = ff_formats_ref(ff_make_format_list(pix_fmts), &ctx->inputs[MAIN]->out_formats);
    if (ret < 0)
        return ret;

    ret = ff_formats_ref(ff_make_format_list(pix_fmts), &ctx->inputs[OVERLAY]->out_formats);
    if (ret < 0)
        return ret;

    ret = ff_formats_ref(ff_make_format_list(pix_fmts), &ctx->outputs[0]->in_formats);
    if (ret < 0)
        return ret;
