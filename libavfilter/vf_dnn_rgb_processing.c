...
static const AVOption dnn_rgb_processing_options[] = {
    ...
    { "fmt",         "AVPixelFormat of the frame", OFFSET(fmt),              AV_OPT_TYPE_PIXEL_FMT, { .i64=AV_PIX_FMT_NONE }, AV_PIX_FMT_NONE, AV_PIX_FMT_NB - 1, FLAGS },
    { NULL }
};
...
static av_cold int init(AVFilterContext *context)
{
    ...
    int supported = 0;
    // support more formats such as AV_PIX_FMT_0RGB if necessary
...
static int filter_frame(AVFilterLink *inlink, AVFrame *in)
{
    ...
    out->height = ctx->output.height;
    out->width  = ctx->output.width;
    ...
    if (ctx->input.dt == DNN_FLOAT) {
        ...
        for (int i = 0; i < out->height; i++) {
            for(int j = 0; j < out->width * 3; j++) {
                ...
                out->data[0][k] = av_clip((int)(dnn_output[t] * 255.0), 0, 255);
            }
        }
    } else {
