...
typedef struct DnnRgbProcessingContext {
    ...
    // the input info from model file
    DNNData model_input;
...
static int config_input(AVFilterLink *inlink)
{
    ...
    result = ctx->model->get_input(ctx->model->model, &ctx->model_input, ctx->model_inputname);
    ...
    if (ctx->model_input.height != -1 && ctx->model_input.height != inlink->h) {
        av_log(ctx, AV_LOG_ERROR, "the model requires frame height %d but got %d\n",
                                   ctx->model_input.height, inlink->h);
        return AVERROR(EIO);
    }
    if (ctx->model_input.width != -1 && ctx->model_input.width != inlink->w) {
        av_log(ctx, AV_LOG_ERROR, "the model requires frame width %d but got %d\n",
                                   ctx->model_input.width, inlink->w);
        return AVERROR(EIO);
    }

    if (ctx->model_input.channels != 3) {
        av_log(ctx, AV_LOG_ERROR, "the model requires input channels %d\n",
                                   ctx->model_input.channels);
        return AVERROR(EIO);
    }
    if (ctx->model_input.dt != DNN_FLOAT && ctx->model_input.dt != DNN_UINT8) {
    ...
    ctx->input.dt = ctx->model_input.dt;
...
static int filter_frame(AVFilterLink *inlink, AVFrame *in)
{
    ...
    if (ctx->model_input.dt == DNN_FLOAT) {
        float *dnn_input = ctx->input.data;
        ...
        av_assert0(ctx->model_input.dt == DNN_UINT8);
        for (int i = 0; i < in->height; i++) {
            for(int j = 0; j < in->width * 3; j++) {
                int k = i * in->linesize[0] + j;
                int t = i * in->width * 3 + j;
    ...
    if (ctx->output.dt == DNN_FLOAT) {
        float *dnn_output = ctx->output.data;
        ...
        uint8_t *dnn_output = ctx->output.data;
        av_assert0(ctx->output.dt == DNN_UINT8);
        for (int i = 0; i < out->height; i++) {
            for(int j = 0; j < out->width * 3; j++) {
                int k = i * out->linesize[0] + j;
                int t = i * out->width * 3 + j;
